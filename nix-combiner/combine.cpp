...
const std::string PROGRAM_PATH =
    "/home/cricro/nixos-config/programs/flake-init/";
...
void process_base_nix(const std::string &base_filepath,
                      const std::vector<std::string> &pkgs,
                      const std::vector<std::string> &inithooks,
  ...
  while (std::getline(base_file, line)) {
    bool replaced = false;
    if (line.find("basepkgs") != std::string::npos) {
      replaced = true;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    process_base_nix(base_nix_filepath, pkgs, inithooks, flake_nix_filepath);
